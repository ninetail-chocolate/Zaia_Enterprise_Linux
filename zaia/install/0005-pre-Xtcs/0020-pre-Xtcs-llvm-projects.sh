...
RANLIB=llvm-ranlib			\
CC=clang				\
CFLAGS="-march=znver1 -O2 -pipe -fPIC -stdlib="libc++" -rtlib="compiler-rt" -unwindlib="libunwind" -D_LIBCPP_HAS_MUSL_LIBC --sysroot=$ZEL -flto=full"	\
CXX=clang++			\
CXXFLAGS="-march=znver1 -O2 -pipe -fPIC -stdlib="libc++" -rtlib="compiler-rt" -unwindlib="libunwind" -D_LIBCPP_HAS_MUSL_LIBC --sysroot=$ZEL -flto=full"	\
cmake												\
-DCMAKE_SYSTEM_NAME="Linux"									\
-DLLVM_TABLEGEN="$ZEL/tools/bin/llvm-tblgen"				\
-DCLANG_TABLEGEN="$ZEL/tools/bin/clang-tblgen"				\
-DLLVM_CONFIG_PATH="$ZEL/tool/bin/llvm-config"				\
-DLLVM_ENABLE_LLD="ON"										\
...
-DLLVM_BINUTILS_INCDIR="$ZEL/usr/include"							\
-Wno-dev											\
-G Ninja											\
...
-DBacktrace_LIBRARY=$ZEL/usr/lib/libexecinfo.so							\
-DBacktrace_INCLUDE_DIR=$ZEL/usr/include							\
...
ln -svf clang $ZEL/usr/bin/cc
ln -svf clang++ $ZEL/usr/bin/c++
ln -svf clang-cpp $ZEL/usr/bin/cpp
